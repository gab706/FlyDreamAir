        ...
        const html = `
            <div class="flight-details">
                <div class="flight-route">
                    ...
                    <span><strong>${escapeHTML(flight.origin)}</strong> → <strong>${escapeHTML(flight.destination)}</strong></span>
                </div>
                ...
                <div class="flight-meta-row">
                    <div class="flight-meta">
                        ...
                        <span>Status: ${escapeHTML(flight.status)}</span>
                    </div>
                    <div class="flight-meta">
    ...
    function renderRewardsPage(page = 1) {
        ...
        pageRewards.forEach(r => {
            ...
            <div class="dashboard-reward-card ${eligible ? '' : 'disabled'}">
                ...
                <div class="dashboard-reward-body">
                    ...
                    <p class="dashboard-reward-desc">${escapeHTML(r.description)}</p>
                    <p class="dashboard-reward-type"><strong>Type:</strong> ${escapeHTML(r.type)}</p>
                    <div class="dashboard-reward-meta">
                        <span class="tier-pill ${escapeHTML(r.tier.toLowerCase())}">${escapeHTML(r.tier)}</span>
                        ${r.purchasable ? `<span class="dashboard-reward-price">${escapeHTML(r.price)} pts</span>` : ''}
                    </div>
                    ${!eligible ? `
        ...
        if (totalPages > 1) {
            for (let i = 1; i <= totalPages; i++) {
                ...
                $pagination.append(`<a href="#" class="reward-page-btn ${active}" data-page="${i}">${i}</a> `);
            }
        }
    }
