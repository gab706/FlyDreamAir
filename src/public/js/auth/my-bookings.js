(async () => {
    ...
    $('#book-new-flight-btn').on('click', () => {
        location.href = '/new-booking';
    });
            ...
            <div class="booking-card-large">
                <div class="booking-card-header">
                    ...
                    <span class="booking-status ${escapeHTML(flight.status.toLowerCase().replace(/\s+/g, '-'))}">
                        <i class="fas fa-info-circle"></i> ${escapeHTML(flight.status)}
                    </span>
                </div>
                ...
                ${!departed && !cancelled ? `
                    <button class="cancel-btn cancel-flight" data-booking-id="${escapeHTML(booking.bookingID)}" data-flight-id="${escapeHTML(flight.flightID)}">
