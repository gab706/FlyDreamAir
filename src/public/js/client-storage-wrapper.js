class ClientStorageWrapper {
    ...
    static _setCookie(k, v, d, secure = false) {
        ...
        const secureAttr = secure ? '; Secure; SameSite=Strict' : '; SameSite=Strict';
    ...
    static _getCookie(k) {
        ...
        for (const c of cookies) {
            ...
            if (index === -1) continue;
            const key = decodeURIComponent(c.slice(0, index));
            const value = decodeURIComponent(c.slice(index + 1));
    ...
    static _removeCookie(k) {
        document.cookie = `${encodeURIComponent(k)}=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; SameSite=Strict`;
    }

    static _clearCookies() {
        document.cookie.split('; ').forEach(c => this._removeCookie(c.split('=')[0]));
    }
    ...
    static async get(key, type = 'local') {
        ...
        else if (type === 'cookie' && this._cookiesOn()) val = this._getCookie(key);
    ...
    static async getAllIndexedDBItems() {
        return new Promise((resolve, reject) => {
            const request = indexedDB.open('FlyDreamAirStorage', 1);
            request.onsuccess = () => {
